...
if st.button('우울도 예측'):  # 사용자가 '우울도 예측' 버튼을 누르면
    ...
    if p < 1:  # p가 1보다 작으면
        ...
        st.write('평범해보여요. 지금처럼만 살면 충분히 행복해질 수 있어요^^')
    elif p < 3:
        st.write('심리상태가 별로 좋지 않아보여요. 밖에 나가 사람을 만나보는건 어떨까요?')
    elif p < 4:
        st.write('혹시 힘든일일 있나요? 주변인에게 고민을 얘기해보세요')
    else:
        st.write('너무 우울해보여요ㅠㅠ. 혹시 병원을 가보는건 어떤가요?')
